...
function drawCanvas() {
    ...
    background.onload = function(){
        ...
        if (gameStarted == false && gameIsOver == false) {
            drawMenu();
        }
...
function drawMenu() {
        ...
        ctx.fillText("Play", canvas.width/2, canvas.height/3);
...
function gameOver() {
        ...
        ctx.fillStyle = "Black";
        ctx.textAlign = "center";
        ctx.fillText("Your final score: " + score, canvas.width/2, canvas.height/3);
        ctx.fillText("Play again", canvas.width/2, canvas.height/2);
        ctx.fillText("Go to menu", canvas.width/2, 2 * canvas.height/3);
        ctx.globalAlpha = 0.2;
        ctx.fillRect(canvas.width/5, canvas.height/4, 3 * canvas.width/5, canvas.height/2);
        ctx.globalAlpha = 1.0;
}
